...
export const getOrCreateConversation = internalMutation({
  ...
  handler: async (ctx, { participantOneId, participantTwoId }): Promise<Id<"conversations">> => {
...
export const getOrCreateConversationWithParticipant = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const otherUserProfile = await ctx.db
      .query("profiles")
      .withIndex("by_userId", (q) => q.eq("userId", args.otherParticipantUserId))
      .unique();

    let otherUserAvatarFinalUrl: string | null = null;
    if (otherUserProfile?.avatarUrl) {
      try {
        otherUserAvatarFinalUrl = await ctx.storage.getUrl(otherUserProfile.avatarUrl as Id<"_storage">);
      } catch (e) {
        console.error(`Failed to get URL for other user avatar storageId ${otherUserProfile.avatarUrl}`, e);
      }
    } else if (otherUserAuth?.image) {
      otherUserAvatarFinalUrl = otherUserAuth.image;
    }
    ...
    return {
      ...
      otherParticipant: {
        ...
        displayName: otherUserProfile?.displayName ?? otherUserAuth?.name ?? "Unknown User",
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, args): Promise<Id<"messages">> => {
    const senderId: Id<"users"> = await getAuthenticatedUserId(ctx);
    ...
    const messagesWithAuthorsAndUrls = await Promise.all(
      result.page.map(async (message) => {
        ...
        if (message.format === "image" || message.format === "video") {
          if (message.body) { 
            try {
                attachmentUrl = await ctx.storage.getUrl(message.body as Id<"_storage">);
            } catch (e) {
                console.error(`Failed to get URL for storageId ${message.body}`, e);
                attachmentUrl = null; 
            }
          }
        }
        ...
        if (authorProfile?.avatarUrl) {
            try {
                authorAvatarFinalUrl = await ctx.storage.getUrl(authorProfile.avatarUrl as Id<"_storage">);
            } catch (e) {
                console.error(`Failed to get URL for avatar storageId ${authorProfile.avatarUrl}`, e);
            }
        } else if (authorUser?.image) {
            authorAvatarFinalUrl = authorUser.image;
        }
        ...
        return {
          ...
          author: {
            ...
            displayName: authorProfile?.displayName ?? authorUser?.name ?? "Unknown User",
...
export const listConversations = query({
  ...
  handler: async (ctx, args) => { 
        ...
        if (conv.lastMessageId) {
          ...
          if (lastMessage) {
            ...
            if (lastMessage.format === "text") {
              lastMessageSnippet = lastMessage.body.substring(0, 30) + (lastMessage.body.length > 30 ? "..." : "");
    ...
    const page = conversationsWithDetails.slice(startIndex, startIndex + numItems);
    const newCursor = (startIndex + page.length < conversationsWithDetails.length) 
